...
if (!function_exists('land_tidy_tree')) {
	 ...
	 * 对树形结构进行整理
	 */
	function land_tidy_tree(array|Collection $tree, callable $callback, string $children_key = 'children'): array|Collection
	{
	...
	function land_predict_date_time_format($target, ?string $type): string
	{
		...
		if (!isset($type) || $type === 'dateTime' || $type === 'date') {
			if (Str::contains($target, ' ')) {
				[$date_part, $time_part] = explode(' ', $target);
			} else {
				$date_part = $target;
			}
			$split = '';
...
if (!function_exists('land_config')) {
	 ...
	 */
	function land_config(array|string|null $key = null, mixed $default = null): mixed
	{
