    ...
    },
    {
        ...
        "children": [
            ...
            },
            {
                ...
                "name": "九龙",
                "children": [
                    { "code": "810201", "name": "九龙城区" },
                    { "code": "810202", "name": "油尖旺区" },
                    { "code": "810203", "name": "深水埗区" },
...
AREA;
        ...
        $area_items = json_decode($area_json, true);

        $this->handleItems($area_items);

    }

    private function handleItems($area_items, $parent = null)
    {
        foreach ($area_items as $area_item) {
            if (Str::endsWith($area_item['code'], '0000')) {
                $area_item['short_code'] = Str::substr($area_item['code'], 0, 2);
            } elseif (Str::endsWith($area_item['code'], '00')) {
                $area_item['short_code'] = Str::substr($area_item['code'], 0, 4);
            } else {
                $area_item['short_code'] = $area_item['code'];
            }

            if (empty($parent)) {
                $area_item['alias'] = $area_item['name'];
            } else {
                $area_item['alias'] = $parent->alias.$area_item['name'];
                $area_item['parent_id'] = $parent->id;
            }

            $children = [];

            if (isset($area_item['children'])) {
                $children = $area_item['children'];
                unset($area_item['children']);
            }

            $area = Area::create($area_item);

            if (count($children)) {
                $this->handleItems($children, $area);
            }
        }
    }
}
